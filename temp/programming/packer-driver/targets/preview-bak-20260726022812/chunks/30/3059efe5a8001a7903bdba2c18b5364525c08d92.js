    ...
    execute: function () {
      ...
      _export("BattlefieldEvaluator", BattlefieldEvaluator = class BattlefieldEvaluator {
        ...
        chooseSnapshotSpawnDecision(gameManager, team, affordableEntries, maxRangedSupportPerTarget, blockedMeleeLaneId, decisionAccuracy) {
          ...
          var cpStrategyState = this.getCPStrategyState(gameManager, team, affordableEntries, maxRangedSupportPerTarget, blockedMeleeLaneId, currentCombatPoint, enemyCombatPoint, rangedSupportCount, meleeSupportCount, decisionAccuracy);
          ...
          for (var i = 0; i < this.enemyCount; i++) {
            ...
            for (var j = 0; j < affordableEntries.length; j++) {
              ...
              var score = this.scoreSnapshotEntryForTarget(gameManager, team, entry, target, targetPriority, currentCombatPoint, enemyCombatPoint, rangedSupportCount, meleeSupportCount, maxRangedSupportPerTarget, hasFullStrengthRangedHardCounter, cpStrategyState, decisionAccuracy);
        ...
        getFallbackCPStrategyState(gameManager, team, affordableEntries, maxRangedSupportPerTarget, blockedMeleeLaneId, decisionAccuracy) {
          ...
          var rangedSupportCount = this.countRangedSupportAllies();
          var meleeSupportCount = this.countMeleeWaves(this.allies, this.allyCount);
          return this.getCPStrategyState(gameManager, team, affordableEntries, maxRangedSupportPerTarget, blockedMeleeLaneId, currentCombatPoint, enemyCombatPoint, rangedSupportCount, meleeSupportCount, decisionAccuracy);
        }

        getCPStrategyState(gameManager, team, affordableEntries, maxRangedSupportPerTarget, blockedMeleeLaneId, currentCombatPoint, enemyCombatPoint, rangedSupportCount, meleeSupportCount, decisionAccuracy) {
          ...
          if (!this.hasAffordableEffectiveResponse(gameManager, team, affordableEntries, maxRangedSupportPerTarget, blockedMeleeLaneId, rangedSupportCount, meleeSupportCount, decisionAccuracy)) {
        ...
        hasAffordableEffectiveResponse(gameManager, team, affordableEntries, maxRangedSupportPerTarget, blockedMeleeLaneId, rangedSupportCount, meleeSupportCount, decisionAccuracy) {
          for (var i = 0; i < this.enemyCount; i++) {
            var target = this.enemies[i];
            ...
            for (var j = 0; j < affordableEntries.length; j++) {
              ...
              if (this.isRangedFamily(entry.family) && !this.isSnapshotRangedSupportAllowed(entry, target, rangedSupportCount, meleeSupportCount, maxRangedSupportPerTarget, hasFullStrengthRangedHardCounter, decisionAccuracy)) {
                continue;
              }
        ...
        chooseFallbackSpawnDecision(gameManager, team, affordableEntries, maxRangedSupportPerTarget, blockedMeleeLaneId, decisionAccuracy) {
          ...
          if (laneId >= 0) {
            if (cpStrategyState === CPStrategyState.Desperate) {
              this.addDesperateFallbackCandidates(affordableEntries, laneId, cpStrategyState, decisionAccuracy, maxRangedSupportPerTarget);
            } else {
        ...
        addDesperateFallbackCandidates(affordableEntries, laneId, cpStrategyState, decisionAccuracy, maxRangedSupportPerTarget) {
          var rangedSupportCount = this.countRangedSupportAllies();
          var meleeSupportCount = this.countMeleeWaves(this.allies, this.allyCount);
          ...
          for (var i = 0; i < affordableEntries.length; i++) {
            ...
            if (ranged && (!this.hasRangedSupportCapacity(rangedSupportCount, meleeSupportCount) || !this.hasRangedSupportBudget(rangedSupportCount, maxRangedSupportPerTarget, decisionAccuracy, true))) {
              continue;
            }

            var power = this.getEntryBasePower(entry, Math.max(1, entry.unitCount), 1, 1);
            var cost = Math.max(1, entry.combatPointCost);
            var rangedBonus = ranged ? 35 : 0;
            var score = power * 0.35 - cost * 0.15 + entry.maxSpeed * 3 + rangedBonus - this.getPressureCavalrySpearLanePenalty(entry, laneId, decisionAccuracy) + Math.random() * 0.001;
            this.addSpawnCandidate(entry, null, laneId, !ranged, 'snapshot-desperate-fallback', score, cpStrategyState);
          }
        }
        ...
        chooseFallbackRangedSupportDecision(gameManager, team, affordableEntries, maxRangedSupportPerTarget, decisionAccuracy) {
          ...
          var rangedSupportCount = this.countRangedSupportAllies();
          var meleeSupportCount = this.countMeleeWaves(this.allies, this.allyCount);
          var cpStrategyState = this.getCPStrategyState(gameManager, team, affordableEntries, maxRangedSupportPerTarget, -1, currentCombatPoint, enemyCombatPoint, rangedSupportCount, meleeSupportCount, decisionAccuracy);
          ...
          for (var i = 0; i < this.enemyCount; i++) {
            ...
            for (var j = 0; j < affordableEntries.length; j++) {
              ...
              var score = this.scoreSnapshotEntryForTarget(gameManager, team, entry, target, 0, currentCombatPoint, enemyCombatPoint, rangedSupportCount, meleeSupportCount, maxRangedSupportPerTarget, hasFullStrengthRangedHardCounter, cpStrategyState, decisionAccuracy);
              if (!Number.isFinite(score)) continue;
        ...
        scoreSnapshotEntryForTarget(gameManager, team, entry, target, targetPriority, currentCombatPoint, enemyCombatPoint, rangedSupportCount, meleeSupportCount, maxRangedSupportPerTarget, hasFullStrengthRangedHardCounter, cpStrategyState, decisionAccuracy) {
          ...
          if (ranged) {
            if (!this.isSnapshotRangedSupportAllowed(entry, target, rangedSupportCount, meleeSupportCount, maxRangedSupportPerTarget, hasFullStrengthRangedHardCounter, decisionAccuracy)) {
              return -Infinity;
            }
          }
        ...
        isSnapshotRangedSupportAllowed(entry, target, rangedSupportCount, meleeSupportCount, maxRangedSupportPerTarget, hasFullStrengthRangedHardCounter, decisionAccuracy) {
          ...
          if (!this.hasRangedSupportBudget(rangedSupportCount, maxRangedSupportPerTarget, decisionAccuracy, !fullStrengthHardCounter)) {
            return false;
          }
        ...
        hasRangedSupportBudget(rangedSupportCount, maxRangedSupportPerTarget, decisionAccuracy, respectOpponentRanged) {
          var maxSupport = Math.max(0, Math.floor(maxRangedSupportPerTarget));

          if (maxSupport <= 0) {
            return false;
          }

          var enemyRangedCount = this.countRangedEnemies();
          var accuracy = this.clamp01(decisionAccuracy);
          var accuracyLimit = maxSupport * accuracy;
          var enemyAwareLimit = respectOpponentRanged ? Math.min(maxSupport, enemyRangedCount) : maxSupport;
          var desiredLimit = Math.min(accuracyLimit, maxSupport * (1 - accuracy) + enemyAwareLimit * accuracy);
          var guaranteedLimit = Math.floor(desiredLimit);

          if (rangedSupportCount < guaranteedLimit) {
            return true;
          }
          ...
          return Math.random() < desiredLimit - guaranteedLimit;
        }
