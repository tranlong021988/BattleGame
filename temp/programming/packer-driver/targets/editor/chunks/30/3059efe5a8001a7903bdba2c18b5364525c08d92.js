      ...
      _export("BattlefieldEvaluator", BattlefieldEvaluator = class BattlefieldEvaluator {
        ...
        chooseEntryForTarget(gameManager, team, target, affordableEntries) {
          ...
          for (let i = 0; i < affordableEntries.length; i++) {
            ...
            const enough = coveragePower >= neededPower;
            ...
            const overshootPenalty = Math.max(0, projectedCoverageRatio - this.coverageTargetRatio) * 140;
            const sufficientBonus = enough ? 120 : 0;
            const score = sufficientBonus + efficiency * 120 + Math.min(180, coveragePower / Math.max(1, neededPower) * 80) - cost * 0.75 - overshootPenalty;
        ...
        getMatchupFactor(entry, target) {
          ...
          if (counter) {
            const counterScore = counter.getCounterScore(attacker, defender);
