      ...
      _export("TrueMiniMapPanel", TrueMiniMapPanel = (_dec4 = ccclass('TrueMiniMapPanel'), _dec5 = property(_crd && GameManager === void 0 ? (_reportPossibleCrUseOfGameManager({
        ...
        syncWithBattleWaves() {
          ...
          for (let i = 0; i < waves.length; i++) {
            ...
            const scan = this.scanWaveForMiniMap(wave, this.tempWorldPosition);
            ...
            const target = this.getMiniMapPositionFromWorldPosition(this.tempWorldPosition);
        ...
        createIconForWave(wave, target, aliveRatio, engaged) {
          ...
          node.setScale(0, 0, 1);
          node.active = true;
          const displayedAliveRatio = this.showAliveRatio ? aliveRatio : 1;
        ...
        createIconForHero(team, hero, target) {
          const root = team === 0 ? this.teamAIconRoot : this.teamBIconRoot;
          ...
          node.setScale(0, 0, 1);
          node.active = true;
          const aliveRatio = hero.props ? hero.props.getHealthRatio() : 1;
          const displayedAliveRatio = this.showAliveRatio ? aliveRatio : 1;
          item.setAliveRatio(displayedAliveRatio);
        ...
        updateTargetsAndState() {
          ...
          this.records.forEach((record, waveId) => {
            ...
            const displayedAliveRatio = this.showAliveRatio ? record.aliveRatio : 1;
        ...
        updateIconNodePosition(record, deltaTime) {
          ...
          const dx = record.targetPosition.x - current.x;
          const dy = record.targetPosition.y - current.y;
        ...
        resolveIconOverlaps() {
          const minSpacing = Math.max(0, this.minIconSpacing);

          if (minSpacing <= 0) {
            return;
          }

          const records = this.iconSeparationRecords;
          records.length = 0;
          ...
          const iterations = Math.max(1, Math.floor(this.iconSeparationIterations));
        ...
        separateIconPair(a, b, minSpacing, minSpacingSq) {
          ...
          if (distSq <= 0.0001) {
            const angle = this.getBoundaryAwarePairAngle(a, b);
            nx = Math.cos(angle);
            ny = Math.sin(angle);
          } else {
        ...
        getIconPairAngle(waveAId, waveBId) {
          const seed = Math.abs((waveAId + 1) * 97 + (waveBId + 1) * 53);
          return seed % 360 * 0.017453292519943295;
        }

        getBoundaryAwarePairAngle(a, b) {
          const baseAngle = this.getIconPairAngle(a.pairId, b.pairId);

          if (!this.clampIconToMapBounds) {
            return baseAngle;
          }

          const midX = (a.rawPosition.x + b.rawPosition.x) * 0.5;
          const midY = (a.rawPosition.y + b.rawPosition.y) * 0.5;
          const padding = this.getIconBoundsPadding();
          const halfWidth = Math.max(0, this.mapWidth * 0.5 - padding);
          const halfHeight = Math.max(0, this.mapHeight * 0.5 - padding);
          const edgeRange = Math.max(this.minIconSpacing, padding + 1);
        ...
        releaseHeroIcon(team) {
          const record = this.heroRecords.get(team);
        ...
        getMiniMapPositionFromWorldPosition(worldPosition) {
          ...
          let x01 = (worldPosition.x - minX) / width;
          let z01 = (worldPosition.z - minZ) / height;
        ...
        scanWaveForMiniMap(wave, out) {
          ...
          const representative = wave.getRepresentativeUnit();
        ...
        createIconNode() {
          ...
          const spriteNode = new Node('mini-map-wave-icon-sprite');
          ...
          const item = node.addComponent(_crd && BattleInformationIconItem === void 0 ? (_reportPossibleCrUseOfBattleInformationIconItem({
            error: Error()
          }), BattleInformationIconItem) : BattleInformationIconItem);
        ...
        getSpriteFrame(team, family) {
          ...
          const spriteFrame = this.findSpriteFrameInList(list, family);
        ...
        getHeroSpriteFrame(team, hero) {
          const heroSpriteFrame = team === 0 ? this.teamAHeroIcon : this.teamBHeroIcon;

          if (heroSpriteFrame) {
            return heroSpriteFrame;
          }

          const family = hero.props ? hero.props.family : (_crd && UnitFamily === void 0 ? (_reportPossibleCrUseOfUnitFamily({
            error: Error()
...
//# sourceMappingURL=ce85f72a100c6a8e39935bb038e3a3493b4e1116.js.map