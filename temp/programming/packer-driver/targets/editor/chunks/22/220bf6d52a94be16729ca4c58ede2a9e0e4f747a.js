  ...
  return {
    ...
    execute: function () {
      ...
      _export("BattleTelemetry", BattleTelemetry = class BattleTelemetry {
        ...
        configureDiagnostics(maxSnapshots, maxDiagnosticEvents, maxScannerTraces = this.maxScannerTraces) {
        ...
        recordLineReachedContext(context) {
          if (!this.isEnabled()) return;
          if (!context) return;
          this.lineReachedContext = { ...context,
        ...
        setFramePerformance(performance) {
          this.framePerformance = performance ? { ...performance
          } : null;
        }
        ...
        recordTargetWaveTransition(event) {
          ...
          if (!this.isEnabled()) return;
          if (!event) return;
          const waveId = Math.max(0, Math.floor((_event$waveId2 = event.waveId) != null ? _event$waveId2 : 0));
          const previousFrame = this.targetWaveTransitionLastFrames.get(waveId);
          const transitionCount = ((_this$targetWaveTrans = this.targetWaveTransitionCounts.get(waveId)) != null ? _this$targetWaveTrans : 0) + 1;
        ...
        recordCardEvent(event) {
          ...
          if (!event) return;
          const normalized = {
            type: event.type === 'card-depleted' ? 'card-depleted' : 'card-activated',
            team: this.clampTeam(event.team),
            id: event.id || '',
            displayName: event.displayName || '',
            budgetRemaining: Math.max(0, Number.isFinite(event.budgetRemaining) ? event.budgetRemaining : 0),
            budgetUsed: Math.max(0, Number.isFinite(event.budgetUsed) ? event.budgetUsed : 0),
        ...
        recordDamage(attacker, victim, damage, actualDamage, isCounterDamage, isAreaDamage = false, attackBatchId = -1, frame = -1, time = 0) {
          if (!this.isEnabled()) return;
        ...
        finish(winnerTeam, loserTeam, reason, frame, time, combatPoint, aliveCount, deathCount, killCount, counterKillCount, progressionResult) {
          ...
          return {
            ...
            diagnostics: {
              ...
              targetWaveTransitions: { ...this.targetWaveTransitionStats
              },
              combatEscalation: { ...this.combatEscalationStats,
            ...
            lineReachedContext: this.lineReachedContext ? { ...this.lineReachedContext,
              targetWaveIds: this.lineReachedContext.targetWaveIds.slice()
            } : null,
            breakthroughCashouts: this.breakthroughCashouts.map(cashout => ({ ...cashout
            })),
            unitTypes
          };
        }
        ...
        recordAttackBatchHit(attackerStats, attackerTeam, attackerWaveId, attackBatchId, isAreaDamage, damage) {
          ...
          const key = `${attackerTeam}:` + `${attackerWaveId}:` + `${Math.floor(attackBatchId)}`;
        ...
        normalizeWaveSpawnDecision(decision) {
          ...
          return {
            ...
            decisionCandidateCount: Number.isFinite(decision.decisionCandidateCount) ? Math.floor(decision.decisionCandidateCount) : undefined,
            decisionSelectedRank: Number.isFinite(decision.decisionSelectedRank) ? Math.floor(decision.decisionSelectedRank) : undefined,
            decisionSelectedScore: Number.isFinite(decision.decisionSelectedScore) ? decision.decisionSelectedScore : undefined,
            decisionBestScore: Number.isFinite(decision.decisionBestScore) ? decision.decisionBestScore : undefined,
            decisionSelectionQuality: Number.isFinite(decision.decisionSelectionQuality) ? decision.decisionSelectionQuality : undefined,
            decisionQualityRatio: Number.isFinite(decision.decisionQualityRatio) ? decision.decisionQualityRatio : undefined,
...
//# sourceMappingURL=220bf6d52a94be16729ca4c58ede2a9e0e4f747a.js.map