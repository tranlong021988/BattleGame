    ...
    execute: function () {
      ...
      _export("PlayerArmyController", PlayerArmyController = (_dec4 = ccclass('PlayerArmyController'), _dec5 = property(_crd && GameManager === void 0 ? (_reportPossibleCrUseOfGameManager({
        ...
        update(deltaTime) {
          ...
          const waiting = (_this$getGameManager$ = (_this$getGameManager2 = this.getGameManager()) == null ? void 0 : _this$getGameManager2.isWaitingForPlayerStart()) != null ? _this$getGameManager$ : false;
        ...
        handleLaneTap(laneId) {
          ...
          if (this.isMaxAliveWaveBlocked()) {
            this.clearPendingLaneTap();
            const manager = this.getGameManager();
            const aliveCount = manager ? manager.getAliveWaveCount(this.team) : 0;
          ...
          if (this.pendingLaneTapTimer > 0 && this.pendingLaneTapLaneId === laneId) {
            const unitName = this.pendingLaneTapUnitName || this.selectedUnitName;
            this.clearPendingLaneTap();
        ...
        flushPendingLaneTap() {
          const unitName = this.pendingLaneTapUnitName || this.selectedUnitName;
          const laneId = this.pendingLaneTapLaneId;
          this.clearPendingLaneTap();
          if (!unitName) return;
        ...
        getMaxAliveWaves() {
          ...
          const progressionLimit = (_this$getGameManager3 = this.getGameManager()) == null || (_this$getGameManager3 = _this$getGameManager3.battleProgressionProvider) == null || _this$getGameManager3.getPlayerMaxAliveWaves == null ? void 0 : _this$getGameManager3.getPlayerMaxAliveWaves();
        ...
        refreshSpawnAvailability() {
          const blocked = this.isMaxAliveWaveBlocked();
        ...
        shouldRefreshSpawnAvailability() {
          const manager = this.getGameManager();
          const aliveWaveCount = manager && this.enableMaxAliveWaveLimit ? manager.getAliveWaveCount(this.team) : -1;
          const combatPoint = manager ? manager.getCombatPoint(this.team) : NaN;
        ...
        updateUnitIconTint(dimmed, force = false) {
        ...
        refreshUnitIconAffordTint(dimmed) {
          for (let i = 0; i < this.unitIcons.length; i++) {
            const item = this.unitIcons[i];
            const node = item ? item.node : null;
            const unitName = item ? item.unitName.trim() : '';
            const canUse = !!unitName && !dimmed && this.canAffordUnitName(unitName);
        ...
        setSelectedUnit(unitName) {
          ...
          const canPrepareSpawn = canAfford && !this.isCoolingDown() && !maxBlocked; // Keep the unit picker visible until the selected unit can actually
          ...
          for (let i = 0; i < this.unitIcons.length; i++) {
            const item = this.unitIcons[i];
            const node = item ? item.node : null;
            const active = !!safeUnitName && !!item && item.unitName.trim() === safeUnitName;
        ...
        canAffordUnitName(unitName) {
          ...
          const manager = (_this$gameManager3 = this.gameManager) != null ? _this$gameManager3 : (_crd && GameManager === void 0 ? (_reportPossibleCrUseOfGameManager({
            error: Error()
          }), GameManager) : GameManager).instance;
          if (!manager) return false;
        ...
        setLanePickersVisible(visible) {
          const container = this.getLanePickerContainer();
        ...
        getLanePickerContainer() {
          const parent = this.leftPicker ? this.leftPicker.parent : null;
        ...
        setNodeTint(node, active, inactiveTint = PlayerArmyController.inactiveTint) {
          if (!node) return;
...
//# sourceMappingURL=ca43d7214a083b257bb741b899a2ab97fadae29e.js.map