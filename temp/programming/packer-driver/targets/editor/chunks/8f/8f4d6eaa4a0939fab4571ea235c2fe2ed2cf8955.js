    ...
    execute: function () {
      ...
      _export("VATCharacterPlayer", VATCharacterPlayer = (_dec9 = ccclass('VATCharacterPlayer'), _dec10 = property({
        ...
        applyMetadataToMaterial() {
          ...
          const metadata = (_this$metadata = this.metadata) == null ? void 0 : _this$metadata.json;
          ...
          const sharedSlotCount = (_renderer$sharedMater = (_renderer$sharedMater2 = renderer.sharedMaterials) == null ? void 0 : _renderer$sharedMater2.length) != null ? _renderer$sharedMater : 0;
          const slotCount = this.applyAllMaterialSlots ? Math.max(1, sharedSlotCount) : 1;
          ...
          for (let i = 0; i < slotCount; i++) {
            ...
            const material = (_renderer$sharedMater3 = renderer.sharedMaterials) == null ? void 0 : _renderer$sharedMater3[i];
          ...
          if (this.enableLog) {
            console.log(`[VATCharacterPlayer] Clips for ${this.node.name}: ` + this.getAnimationNames(metadata).map((name, index) => {
              ...
              const animation = (_metadata$animations = metadata.animations) == null ? void 0 : _metadata$animations[name];
        ...
        applyMetadata(material, metadata) {
          ...
          const vertexCount = Math.max(1, (_metadata$vertexCount = metadata.vertexCount) != null ? _metadata$vertexCount : 1);
          const frameCount = Math.max(1, (_metadata$frameCount = metadata.frameCount) != null ? _metadata$frameCount : 1);
          const textureWidth = Math.max(1, (_metadata$textureWidt = metadata.textureWidth) != null ? _metadata$textureWidt : vertexCount);
          const textureHeight = Math.max(1, (_metadata$textureHeig = metadata.textureHeight) != null ? _metadata$textureHeig : frameCount);
          const rowsPerFrame = Math.max(1, (_metadata$rowsPerFram = metadata.rowsPerFrame) != null ? _metadata$rowsPerFram : Math.ceil(vertexCount / textureWidth));
          const boundsMin = (_metadata$boundsMin = metadata.boundsMin) != null ? _metadata$boundsMin : [0, 0, 0];
          const boundsMax = (_metadata$boundsMax = metadata.boundsMax) != null ? _metadata$boundsMax : [1, 1, 1];
          const animation = this.resolveAnimation(metadata, frameCount, this.currentAnimationName);
          const fps = Math.max(0, ((_ref = (_animation$fps = animation.fps) != null ? _animation$fps : metadata.fps) != null ? _ref : 30) * this.speedScale);
        ...
        playAnimation(animationName, duration = this.blendDuration) {
          ...
          const frameCount = Math.max(1, (_this$metadataCache$f = this.metadataCache.frameCount) != null ? _this$metadataCache$f : 1);
          ...
          const fromFps = Math.max(0, ((_ref2 = (_fromAnimation$fps = fromAnimation.fps) != null ? _fromAnimation$fps : this.metadataCache.fps) != null ? _ref2 : 30) * this.speedScale);
          const toFps = Math.max(0, ((_ref3 = (_toAnimation$fps = toAnimation.fps) != null ? _toAnimation$fps : this.metadataCache.fps) != null ? _ref3 : 30) * this.speedScale);
        ...
        applyCurrentAnimationUniforms(blendWeight) {
          ...
          const frameCount = Math.max(1, (_this$metadataCache$f2 = this.metadataCache.frameCount) != null ? _this$metadataCache$f2 : 1);
          const animation = this.resolveAnimation(this.metadataCache, frameCount, this.currentAnimationName);
          const fps = Math.max(0, ((_ref4 = (_animation$fps2 = animation.fps) != null ? _animation$fps2 : this.metadataCache.fps) != null ? _ref4 : 30) * this.speedScale);
          const offset = this.currentPlaybackOffset;
        ...
        resolveAnimation(metadata, frameCount, animationName) {
          ...
          const animations = (_metadata$animations2 = metadata.animations) != null ? _metadata$animations2 : {};
        ...
        resolveAnimationName(metadata, animationName, warnIfMissing) {
          ...
          const animations = (_metadata$animations3 = metadata.animations) != null ? _metadata$animations3 : {};
        ...
        updateAnimationEvents() {
          ...
          const frameCount = Math.max(1, (_this$metadataCache$f3 = this.metadataCache.frameCount) != null ? _this$metadataCache$f3 : 1);
          const animation = this.resolveAnimation(this.metadataCache, frameCount, this.currentAnimationName);
          const clipFrameCount = Math.max(1, animation.endFrame - animation.startFrame + 1);
          const fps = Math.max(0, ((_ref6 = (_animation$fps3 = animation.fps) != null ? _animation$fps3 : this.metadataCache.fps) != null ? _ref6 : 30) * this.speedScale);
...
//# sourceMappingURL=8f4d6eaa4a0939fab4571ea235c2fe2ed2cf8955.js.map