    ...
    execute: function () {
      ...
      _export("BattleTelemetry", BattleTelemetry = class BattleTelemetry {
        ...
        recordWaveSpawnEvent(event) {
          if (!this.isEnabled()) return;
          if (!event) return;
          var waveId = Number.isFinite(event.waveId) ? Math.floor(event.waveId) : -1;
        ...
        recordCombatPointEarned(killer, victim, amount, isCounterKill, frame, time) {
          ...
          var earned = Math.max(0, Number.isFinite(amount) ? amount : 0);
          if (earned <= 0) return;
        ...
        recordWaveSpawnDecision(decision) {
          ...
          var targetSpawnFrame = this.waveSpawnFrameById.get(normalized.targetWaveId);
          var targetSpawnTime = this.waveSpawnTimeById.get(normalized.targetWaveId);
        ...
        finish(winnerTeam, loserTeam, reason, frame, time, combatPoint, aliveCount, deathCount, killCount, counterKillCount, progressionResult) {
          ...
          var unitTypes = Array.from(this.unitStats.values()).map(stats => {
            ...
            var spent = Math.max(0.0001, stats.totalCombatPointSpent);
