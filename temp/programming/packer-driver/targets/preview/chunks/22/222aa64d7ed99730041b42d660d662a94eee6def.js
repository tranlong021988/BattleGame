      ...
      _export("ArmyBrain", ArmyBrain = (_dec = ccclass('ArmyBrain'), _dec2 = property(_crd && GameManager === void 0 ? (_reportPossibleCrUseOfGameManager({
        ...
        thinkAndSpawn() {
          ...
          if (!targetWave) {
            ...
            if (this.spawnRandomIfNoThreat) {
              var _randomEntry = this.getRandomEntry(validEntries);

              if (_randomEntry) {
                this.log("Fallback random spawn: " + _randomEntry.name + " / " + (_crd && unitTypeToName === void 0 ? (_reportPossibleCrUseOfunitTypeToName({
                  error: Error()
                }), unitTypeToName) : unitTypeToName)(_randomEntry.unitType));
                this.gameManager.spawnWaveByEntry(this.team, _randomEntry);
              }
            }
        ...
        chooseEntryAgainstWave(entries, targetWave) {
          ...
          var selected = bestEntries[index];
