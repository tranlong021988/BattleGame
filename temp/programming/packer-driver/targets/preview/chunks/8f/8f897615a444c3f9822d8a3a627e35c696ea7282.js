    ...
    execute: function () {
      ...
      _export("LevelSettings", LevelSettings = (_dec8 = ccclass('LevelSettings'), _dec9 = property({
        ...
        constructor() {
          ...
          _initializerDefineProperty(this, "allowCardCooldownAds", _descriptor37, this);

          _initializerDefineProperty(this, "allowBotGoldX2Ads", _descriptor38, this);

          _initializerDefineProperty(this, "progressionStorageKey", _descriptor39, this);

          _initializerDefineProperty(this, "battleCardDeckSize", _descriptor40, this);

          _initializerDefineProperty(this, "enemyCardDiversityScoreFloor", _descriptor41, this);

          _initializerDefineProperty(this, "botStrengthUpgradePurchaseWeight", _descriptor42, this);

          _initializerDefineProperty(this, "maxPlayerPackagesPerLevel", _descriptor43, this);

          _initializerDefineProperty(this, "initialPlayerGold", _descriptor44, this);

          _initializerDefineProperty(this, "playerInitialCPStart", _descriptor45, this);

          _initializerDefineProperty(this, "playerMaxAliveStart", _descriptor46, this);

          _initializerDefineProperty(this, "playerMaxAliveMax", _descriptor47, this);

          _initializerDefineProperty(this, "mainlineNoLossGoldReserve", _descriptor48, this);

          _initializerDefineProperty(this, "bossGoldRewardMultiplier", _descriptor49, this);

          _initializerDefineProperty(this, "sideRewardFeeMultiplier", _descriptor50, this);

          _initializerDefineProperty(this, "sideRecoveryAccuracyMultiplier", _descriptor51, this);

          _initializerDefineProperty(this, "mainBattleEntryFeeRatio", _descriptor52, this);

          _initializerDefineProperty(this, "mainLossRewardFeeRatio", _descriptor53, this);

          _initializerDefineProperty(this, "unitUnlockCostMultiplier", _descriptor54, this);

          _initializerDefineProperty(this, "initialCPGoldPerPoint", _descriptor55, this);

          _initializerDefineProperty(this, "maxAliveBasePrice", _descriptor56, this);

          _initializerDefineProperty(this, "unitProgressionRules", _descriptor57, this);
        ...
        handleBattleResult(winnerTeam, loserTeam, reason) {
          ...
          var lossGold = this.getMainBattleLossReward(battleLevel);
          var goldReward = 0;
        ...
        handleSideMissionBattleResult(winnerTeam, loserTeam, reason) {
          ...
          var state = this.progressionState;
          var before = this.createTelemetrySnapshot();
          this.telemetryActionPhase = 'battle-result'; // Side missions use the same cooldown contract as Main: existing
          ...
          if (winnerTeam === 0) {
            ...
            var recovery = this.getSideEconomyRecoveryStatus(state);
        ...
        createTelemetrySnapshot() {
          ...
          var sideEconomyRecovery = this.getSideEconomyRecoveryStatus(state);
          var sideRecoveryAccuracyMultiplier = sideEconomyRecovery.active ? Math.pow(this.clamp01(this.sideRecoveryAccuracyMultiplier), state.consecutiveSideRecoveryLosses) : 1;
          var noLossEconomyAudit = this.getMainlineNoLossEconomyAudit();
          return {
            enabled: true,
            ...
            player: {
              ...
              cooldownAdReasons: Array.from(this.currentPlayerCooldownAdReasons.entries()).map(_ref => {
                var [cardId, reason] = _ref;
                return {
                  cardId,
                  reason
                };
        ...
        tryFinishCardCooldownWithAd(cardId) {
          if (!this.allowAdsRescue || !this.allowCardCooldownAds || !this.progressionState || !cardId) {
            return false;
          }
        ...
        configureBattleCardsForCurrentBattle() {
          ...
          if (this.purchasingSimulation) {
            ...
            var ownedDefinitions = database.cards.filter(definition => {
              var saved = this.getSavedCard(state, definition.id);
              return !!saved && saved.owned;
            });
            var readyOwnedDefinitions = ownedDefinitions.filter(definition => {
              var saved = this.getSavedCard(state, definition.id);
              return !!saved && saved.cooldownRemaining <= 0;
            });
            var eligibleDefinitions = ownedDefinitions.filter(definition => {
            ...
            var playerCardDefinitions = eligibleDefinitions.length > 0 ? eligibleDefinitions : ownedDefinitions.filter(definition => this.getCardTargetCombatWeight(definition, 0, state) > 0);
            var mirrorCardIds = this.isBossLevelFor(this.battleLevel) ? this.getBossMirrorCardIds(state, playerCardDefinitions) : [];
            var useBossMirror = mirrorCardIds.length === enemyDeckSize;
            var deckDefinitions = useBossMirror ? playerCardDefinitions.filter(definition => mirrorCardIds.includes(definition.id)) : playerCardDefinitions;
            var readyDefinitions = deckDefinitions.filter(definition => {
              var saved = this.getSavedCard(state, definition.id);
              return !!saved && saved.cooldownRemaining <= 0;
            });
            var cooldownDefinitions = deckDefinitions.filter(definition => {
              var saved = this.getSavedCard(state, definition.id);
              return !!saved && saved.cooldownRemaining > 0;
            });
            var cooldownAdPlan = this.selectBotCooldownAdDeck(readyDefinitions, cooldownDefinitions, state, this.currentBattleUsesPreparedDeck || useBossMirror);
            ...
            if (this.currentPlayerBattleCardIds.length <= 0 && this.allowAdsRescue && this.allowCardCooldownAds && ownedDefinitions.length > 0) {
              var emergencyCardIds = this.selectBestPlayerCardIds(ownedDefinitions, state, 1);

              if (emergencyCardIds.length > 0) {
                var emergencyCardId = emergencyCardIds[0];
        ...
        configureEnemyBattleCards(database, state) {
          ...
          var cachedDeck = Array.isArray(savedEnemyDeck) ? savedEnemyDeck.filter(id => {
        ...
        getEnemyBattleCardDeckSizeFor(level) {
          ...
          var nextBossLevel = Math.ceil(safeLevel / bossPace) * bossPace; // A preview exists only when this interval really ends in a boss.
          // This keeps a truncated campaign tail clear of enemy cards.

          if (nextBossLevel > this.getSafeTotalLevels()) return 0;
          var distanceToBoss = nextBossLevel - safeLevel; // The final normal level always previews one card, independent of
        ...
        selectBestPlayerCardIds(definitions, state, maxCount) {
          ...
          var scored = candidates.map(definition => ({
            definition,
        ...
        getBossMirrorCardIds(state, eligibleDefinitions) {
          ...
          var _loop2 = function _loop2() {
            var cardId = _this2.currentEnemyBattleCardIds[i];
            var definition = eligibleDefinitions.find(entry => entry.id === cardId);

            var saved = _this2.getSavedCard(state, cardId);

            if (!definition || !saved || !saved.owned) return 0; // continue

            if (saved.cooldownRemaining > 0 && (!_this2.allowAdsRescue || !_this2.allowCardCooldownAds)) {
              return {
                v: []
              };
            }
          ...
          for (var i = 0; i < this.currentEnemyBattleCardIds.length; i++) {
            _ret = _loop2();
            if (_ret === 0) continue;
        ...
        selectBotCooldownAdDeck(readyDefinitions, cooldownDefinitions, state, forceCompetitivePlan) {
          ...
          if (!this.allowAdsRescue || !this.allowCardCooldownAds || deckSize <= 0) {
            return {
              cardIds: noAdCardIds,
              candidates
            };
          ...
          var bestCardIds = noAdCardIds;
          var bestScore = this.getPlayerDeckScore(noAdCardIds, noAdDefinitions, state);
          var playerStrength = this.getPlayerTeamCombatWeightForCardScore(state);
          var enemyStrength = this.getEnemyTeamCombatWeightForCardScore(state) + this.getEnemyDeckScore(state);
          var missingStrength = Math.max(0, enemyStrength - playerStrength - bestScore);

          for (var i = 0; i < cooldownDefinitions.length; i++) {
            var definition = cooldownDefinitions[i];
            var proposedDefinitions = noAdDefinitions.concat(definition);
            var proposedCardIds = this.selectBestPlayerCardIds(proposedDefinitions, state, deckSize);

            if (!proposedCardIds.includes(definition.id)) {
              continue;
            }

            var proposedScore = this.getPlayerDeckScore(proposedCardIds, proposedDefinitions, state);
            var scoreGain = proposedScore - bestScore;
            var materiallyImprovesDeck = forceCompetitivePlan ? scoreGain > 0 : scoreGain >= Math.max(0.01, missingStrength * 0.15);
        ...
        shouldBotUseCooldownAdPlan(mainLosses, forceCompetitivePlan, cardIds, selectedCandidates, state) {
          var playerTeamStrength = this.getPlayerTeamCombatWeightForCardScore(state);
          var enemyStrength = this.getEnemyTeamCombatWeightForCardScore(state) + this.getEnemyDeckScore(state);
          var strongestCandidate = selectedCandidates.reduce((best, candidate) => Math.max(best, this.getPlayerBattleCardScore(candidate.definition, state)), 0);
          var requiredCardStrength = Math.max(1, enemyStrength - playerTeamStrength);
          var candidateIsIndividuallyMeaningful = strongestCandidate >= requiredCardStrength; // A prepared deck has already shown that this cooling card is the
          ...
          var probability = forceCompetitivePlan ? 0.28 : 0.12;
          probability += Math.min(0.48, Math.max(0, mainLosses) * 0.16);
          if (candidateIsIndividuallyMeaningful) probability += 0.14;
          var clampedProbability = this.clamp01(Math.max(0.08, Math.min(0.86, probability)));
          var useAds = Math.random() < clampedProbability;
          var target = selectedCandidates[0];
          var targetCard = target ? this.getSavedCard(state, target.definition.id) : null;
          this.recordBotSimulationEvent(state, {
        ...
        getCardUpgradeRankLimitAtLevel(definition, upgradeKind, level, offerLevelOffset) {
          if (offerLevelOffset === void 0) {
            offerLevelOffset = 1;
          }
          ...
          var rankLimit = 0;
          var schedule = this.getCardUpgradeSchedule(offerLevelOffset);
        ...
        getCardUpgradeSchedule(offerLevelOffset) {
          if (offerLevelOffset === void 0) {
            offerLevelOffset = 1;
          }
          ...
          for (var _i2 = 0; _i2 < offerWaves.length; _i2++) {
            var _offerWave = offerWaves[_i2];
            ...
            for (var itemIndex = 0; itemIndex < _pending.length; itemIndex++) {
              result.push(_extends({}, _pending[itemIndex], {
                offerLevel: Math.min(totalLevels, offerLevels[Math.min(offerLevels.length - 1, Math.floor(itemIndex * offerLevels.length / _pending.length))] + offerLevelOffset)
              }));
            }
          }
        ...
        applyPlayerPackageOfferCap(upgrades) {
          var cap = Math.max(0, Math.floor(this.maxPlayerPackagesPerLevel));
          if (cap <= 0 || upgrades.length <= 0) return upgrades;
          var totalLevels = this.getSafeTotalLevels();
          var packageCounts = this.getPlayerNonCardUpgradeOfferCounts();
          var ordered = upgrades.map((offer, order) => ({
            offer,
            order
          ...
          for (var index = 0; index < ordered.length; index++) {
            ...
            var offerLevel = Math.max(1, item.offer.offerLevel);
            ...
            result.push(_extends({}, item.offer, {
              offerLevel
            }));
          }
          ...
          try {
            ...
            for (var level = 1; level <= totalLevels; level++) {
              ...
              for (var pass = 0; pass < 1000; pass++) {
                var option = this.getPurchaseOptions(planState, false).slice().sort((a, b) => a.cost - b.cost || a.id.localeCompare(b.id))[0];
                if (!option) break;
                result[level - 1]++;
        ...
        sanitizeProgressionState(source) {
          ...
          var _loop5 = function _loop5() {
            var item = initial.maxAlivePackages[_i3];
          ...
          for (var _i3 = 0; _i3 < initial.maxAlivePackages.length; _i3++) {
            if (_loop5()) continue;
          }
          ...
          for (var _i4 = 0; _i4 < initial.cards.length; _i4++) {
            if (_loop6()) continue;
          }

          var _loop7 = function _loop7() {
            var unit = initial.units[_i5];
          ...
          for (var _i5 = 0; _i5 < initial.units.length; _i5++) {
            if (_loop7()) continue;
          }
        ...
        applyProgressionRuntimeState(syncCurrentCombatPoint) {
          ...
          for (var _i6 = 0; _i6 < playerBrains.length; _i6++) {
            playerBrains[_i6].maxAliveWaves = state.playerMaxAlive;
          }
        }
        ...
        applySideMissionRuntimeState() {
          ...
          var recoveryAccuracyMultiplier = recovery.active ? Math.pow(this.clamp01(this.sideRecoveryAccuracyMultiplier), state.consecutiveSideRecoveryLosses) : 1;

          for (var _i7 = 0; _i7 < enemyBrains.length; _i7++) {
            enemyBrains[_i7].maxAliveWaves = state.playerMaxAlive;

            if (this.allowDecisionAccuracy) {
              enemyBrains[_i7].decisionAccuracy = this.clamp01(baselineAccuracy * recoveryAccuracyMultiplier);
            }
          }
        }
        ...
        configureSideMissionBattleCards() {
          ...
          if (!manager) return;
          var state = this.progressionState;
          var database = manager.battleCardDatabase;
          ...
          if (this.purchasingSimulation) {
            var readyOwnedDefinitions = database.cards.filter(definition => {
              var saved = this.getSavedCard(state, definition.id);
          ...
          manager.configureBattleCardDecks(this.currentPlayerBattleCardIds, [], this.getPlayerCardBudgetUpgradeLevels(state), this.getPlayerCardStrengthScales(state), {}, this.getBattleCardDeckSize(), 0);
        }

        getPurchaseOptions(state, includeCardUpgrades) {
          if (includeCardUpgrades === void 0) {
            includeCardUpgrades = true;
          }
          ...
          if (cardDatabase) {
            for (var _i8 = 0; _i8 < cardDatabase.cards.length; _i8++) {
              var definition = cardDatabase.cards[_i8];
              if (!definition || !definition.id) continue;
        ...
        runPurchaseSimulation(records, source, reservedGold) {
          ...
          if (!this.progressionState) return;
          var preparationTarget = this.getBotPreparationPlan(this.progressionState).target;
          var goldAfterReservedCosts = this.progressionState.playerGold - Math.max(0, reservedGold);
          var targetReserve = preparationTarget && preparationTarget.cost <= goldAfterReservedCosts ? preparationTarget.cost : 0;
          var reserve = Math.max(0, Math.floor(reservedGold + targetReserve));
        ...
        getBotPurchaseCandidates(state, affordableOnly, allowBossMirror) {
          if (allowBossMirror === void 0) {
            allowBossMirror = true;
          }

          var options = this.getPurchaseOptions(state).filter(option => !affordableOnly || option.cost <= state.playerGold);
          var baselineUnitCountOptions = this.getOpenedUnitCountBaselineOptions(options, state); // An already offered unit-count rank is part of the battle
          ...
          if (allowBossMirror && this.isBossLevelFor(this.battleLevel)) {
            var mirrorTarget = this.getNextBossMirrorPurchaseOption(state);
        ...
        getBotPreparationPlan(state) {
          ...
          var enemyStrength = this.getEnemyTeamCombatWeightForCardScore(state) + this.getEnemyDeckScore(state);
          var bossMirrorTarget = this.getNextBossMirrorPurchaseOption(state);
        ...
        getNextBossMirrorPurchaseOption(state) {
          ...
          var _loop8 = function _loop8() {
            var cardId = _this4.currentEnemyBattleCardIds[i];
            var definition = database.getCard(cardId);

            var saved = _this4.getSavedCard(state, cardId);

            if (!definition || !_this4.isCardEligibleForTeam(definition, 0, state)) {
              return 0; // continue
            }

            if (!saved || !saved.owned) {
              var unlock = _this4.getPurchaseOptions(state).find(option => option.kind === 'card-unlock' && option.cardId === cardId);
            ...
            var enemyStrengthRank = _this4.getEnemyStrengthUpgradeRank(definition, _this4.battleLevel);

            if (saved.strengthUpgradeLevel < enemyStrengthRank) {
              var strengthUpgrade = _this4.getPurchaseOptions(state).find(option => option.kind === 'card-strength-upgrade' && option.cardId === cardId);

              if (strengthUpgrade) return {
                v: strengthUpgrade
              };
            }
          },
        ...
        getMainBattleReward(level) {
          var safeLevel = this.clampLevel(level);
          var normalGold = this.getMainBattleNormalReward(safeLevel);
          var gold = this.isBossLevelFor(safeLevel) ? this.getBossMainBattleReward(safeLevel) : normalGold;
          return {
        ...
        getMainBattleNormalReward(level) {
          var safeLevel = this.clampLevel(level);
          var plan = this.getMainBattleNormalGoldPlan();
        ...
        getMainBattleNormalGoldPlan() {
          ...
          var contentFundingBudgets = this.getMainlineContentFundingBudgets();
          var result = [];
          var previousReward = 0;
          var availableGold = Math.max(0, Math.floor(this.initialPlayerGold));
          var targetReserve = Math.max(0, Math.round(Math.max(0, this.mainlineNoLossGoldReserve) / 50) * 50); // Rewards are paced by bossStagePace instead of rising every battle.
          ...
          var _loop9 = function _loop9(level) {
            var entryFee = level <= 1 ? 0 : _this5.getMainBattleEntryFeeForReward(previousReward, level);
            availableGold -= contentFundingBudgets[level - 1] + entryFee;

            var paceStart = level <= 1 || _this5.getRewardPaceIndex(level) !== _this5.getRewardPaceIndex(level - 1);

            var normalReward = paceStart ? previousReward + 50 : previousReward;

            if (paceStart) {
              var paceEnd = Math.min(totalLevels, _this5.getRewardPaceEndLevel(level));

              var canFundPace = candidate => {
                var projectedGold = availableGold;

                for (var projectedLevel = level; projectedLevel <= paceEnd; projectedLevel++) {
                  if (projectedLevel > level) {
                    var projectedFee = _this5.getMainBattleEntryFeeForReward(candidate, projectedLevel);
                  ...
                  projectedGold += _this5.getMainBattleRewardForNormalBase(projectedLevel, candidate);
                }

                if (paceEnd >= totalLevels) return true;
                var nextLevel = paceEnd + 1;

                var nextEntryFee = _this5.getMainBattleEntryFeeForReward(candidate, nextLevel);

                return projectedGold >= contentFundingBudgets[nextLevel - 1] + nextEntryFee + targetReserve;
              };

              if (!canFundPace(normalReward)) {
                var lowStep = Math.ceil(normalReward / 50);
                var highStep = lowStep;
                ...
                while (lowStep < highStep) {
                  var middleStep = Math.floor((lowStep + highStep) / 2);
            ...
            availableGold += _this5.getMainBattleRewardForNormalBase(level, normalReward);
        ...
        getMainBattleRewardForNormalBase(level, normalReward) {
          var safeReward = Math.max(0, normalReward);
        ...
        getMainlineContentFundingBudgets() {
          ...
          try {
            ...
            for (var level = 1; level <= totalLevels; level++) {
              ...
              for (var pass = 0; pass < 1000; pass++) {
                var options = this.getPurchaseOptions(planState).slice().sort((a, b) => a.cost - b.cost || a.id.localeCompare(b.id));
        ...
        getMainlineNoLossEconomyAudit() {
          ...
          var savedState = this.progressionState;
          var savedBattleLevel = this.battleLevel;
          var auditState = this.createInitialProgressionState();
          var minimumGold = auditState.playerGold;
          ...
          try {
            ...
            for (var level = 1; level <= totalLevels; level++) {
              ...
              for (var pass = 0; pass < 1000; pass++) {
                var options = this.getPurchaseOptions(auditState).slice().sort((a, b) => a.cost - b.cost || a.id.localeCompare(b.id));
                var option = options[0];
                if (!option) break;
              ...
              var fee = this.getMainBattleEntryFee(level);
        ...
        getMainBattleLossReward(level) {
          var entryFee = this.getMainBattleEntryFee(level);
          var ratio = this.clamp01(this.mainLossRewardFeeRatio);
          return Math.max(0, Math.round(entryFee * ratio / 10) * 10);
        }

        getMainBattleEntryFeeForReward(reward, level) {
          if (level === void 0) {
            level = 0;
          }

          var baseFee = Math.max(0, reward) * this.clamp01(this.mainBattleEntryFeeRatio);
          var monotonicMinimum = level > 1 ? (level - 1) * 50 : 0;
        ...
        getSideMissionReward() {
          var entryFee = Math.max(50, this.getMainBattleEntryFee(this.battleLevel));
          var multiplier = Math.max(0.1, Math.min(1, this.sideRewardFeeMultiplier));
          var baseGold = Math.max(50, Math.ceil(entryFee * multiplier / 50) * 50);
          return {
            targetId: '',
            targetCost: 0,
            gold: baseGold
        ...
        getSideEconomyRecoveryStatus(state) {
          var entryFee = this.getMainBattleEntryFee(this.battleLevel);
          var options = this.getPurchaseOptions(state);
          var cheapestPackageCost = 0;

          for (var i = 0; i < options.length; i++) {
            var cost = Math.max(0, options[i].cost);
          ...
          var requiredGold = entryFee + cheapestPackageCost;
          var shortfall = Math.max(0, requiredGold - state.playerGold);
          return {
            entryFee,
            cheapestPackageCost,
            requiredGold,
        ...
        grantBotGoldClaim(state, baseGold, type, targetId, targetCost) {
          ...
          var useAds = type === 'side-mission-win' && this.purchasingSimulation && this.allowAdsRescue && this.allowBotGoldX2Ads && state.levelLossCount > 0 && decision.useAds;
        ...
        getUnitCountTailUpgradeSchedule() {
          ...
          while (pending.some(item => item.remaining > 0)) {
            for (var _i10 = 0; _i10 < pending.length; _i10++) {
              var item = pending[_i10];
        ...
        getTargetBattleArmyBrains(team) {
          ...
          for (var _i11 = 0; _i11 < brains.length; _i11++) {
            var _brain = brains[_i11];
            if (!_brain) continue;
        ...
        createMaxAlivePackageSchedule() {
          ...
          for (var _i12 = 0; _i12 < milestones.length; _i12++) {
            var targetLevel = milestones[_i12];
      ...
      }), _descriptor37 = _applyDecoratedDescriptor(_class5.prototype, "allowCardCooldownAds", [_dec35], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor38 = _applyDecoratedDescriptor(_class5.prototype, "allowBotGoldX2Ads", [_dec36], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor39 = _applyDecoratedDescriptor(_class5.prototype, "progressionStorageKey", [_dec37], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor40 = _applyDecoratedDescriptor(_class5.prototype, "battleCardDeckSize", [_dec38], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor41 = _applyDecoratedDescriptor(_class5.prototype, "enemyCardDiversityScoreFloor", [_dec39], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor42 = _applyDecoratedDescriptor(_class5.prototype, "botStrengthUpgradePurchaseWeight", [_dec40], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor43 = _applyDecoratedDescriptor(_class5.prototype, "maxPlayerPackagesPerLevel", [_dec41], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor44 = _applyDecoratedDescriptor(_class5.prototype, "initialPlayerGold", [_dec42], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor45 = _applyDecoratedDescriptor(_class5.prototype, "playerInitialCPStart", [_dec43], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor46 = _applyDecoratedDescriptor(_class5.prototype, "playerMaxAliveStart", [_dec44], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor47 = _applyDecoratedDescriptor(_class5.prototype, "playerMaxAliveMax", [_dec45], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor48 = _applyDecoratedDescriptor(_class5.prototype, "mainlineNoLossGoldReserve", [_dec46], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor49 = _applyDecoratedDescriptor(_class5.prototype, "bossGoldRewardMultiplier", [_dec47], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor50 = _applyDecoratedDescriptor(_class5.prototype, "sideRewardFeeMultiplier", [_dec48], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor51 = _applyDecoratedDescriptor(_class5.prototype, "sideRecoveryAccuracyMultiplier", [_dec49], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor52 = _applyDecoratedDescriptor(_class5.prototype, "mainBattleEntryFeeRatio", [_dec50], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor53 = _applyDecoratedDescriptor(_class5.prototype, "mainLossRewardFeeRatio", [_dec51], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor54 = _applyDecoratedDescriptor(_class5.prototype, "unitUnlockCostMultiplier", [_dec52], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor55 = _applyDecoratedDescriptor(_class5.prototype, "initialCPGoldPerPoint", [_dec53], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor56 = _applyDecoratedDescriptor(_class5.prototype, "maxAliveBasePrice", [_dec54], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
      ...
      }), _descriptor57 = _applyDecoratedDescriptor(_class5.prototype, "unitProgressionRules", [_dec55], {
        configurable: true,
        enumerable: true,
        writable: true,
        initializer: function initializer() {
