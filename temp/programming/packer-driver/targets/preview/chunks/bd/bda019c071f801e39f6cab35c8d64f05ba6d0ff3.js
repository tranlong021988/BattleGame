  ...
  return {
    ...
    execute: function () {
      ...
      _export("BattleCinematicCameraController", BattleCinematicCameraController = (_dec = ccclass('BattleCinematicCameraController'), _dec2 = property(Camera), _dec3 = property(_crd && CinematicOrbitRig === void 0 ? (_reportPossibleCrUseOfCinematicOrbitRig({
        ...
        focusWave(wave) {
          if (!wave) return;
          var unit = wave.getRandomPreferredAliveUnit();
          if (!unit) return;
        ...
        onUnitWillDespawn(unit) {
          ...
          var switched = this.trySwitchTargetBeforeDespawn();
        ...
        validateTarget() {
          ...
          if (!this.isUnitAlive(this.currentUnit)) {
            ...
            var _switched = this.trySwitchTargetBeforeDespawn();
        ...
        trySwitchTargetBeforeDespawn() {
          ...
          var sameWaveUnit = this.currentWave.getRandomPreferredAliveUnit();
        ...
        switchToAnotherWave(team, excludeCurrentWave) {
          ...
          var nextUnit = nextWave.getRandomPreferredAliveUnit();
        ...
        updateCameraLocalToOrbitPose(deltaTime) {
          ...
          var moveDuration = Math.max(0.0001, this.enterMoveDuration);
          var move01 = this.clamp01(this.enterTimer / moveDuration);
          var moveT = this.smooth01(move01);
          ...
          var focusDelay = moveDuration * this.enterFocusDelayRatio;
          var focusDuration = Math.max(0.0001, this.enterFocusDuration);
          var focus01 = this.clamp01((this.enterTimer - focusDelay) / focusDuration);
          ...
          var targetFov = this.orbitRig.getCameraFov();
        ...
        updateReturnToOriginal(deltaTime) {
          ...
          var posDone = Vec3.distance(this.returnCurrentPos, this.originalPos) <= this.returnPositionThreshold;
          var fovDone = Math.abs(this.mainCamera.fov - this.originalFov) <= this.returnFovThreshold;
          var rotDone = focus01 >= 1;
          var moveDone = move01 >= 1;
        ...
        tryFocusUnitAtScreenPoint(screenX, screenY) {
          ...
          if (this.state !== CinematicState.Idle) return;
          var unit = this.pickAliveUnitAtScreenPoint(screenX, screenY);
          if (!unit) return;
          this.focusUnit(unit);
        }
        ...
        pickAliveUnitAtScreenPoint(screenX, screenY) {
          ...
          var aResult = this.pickClosestAliveUnitInList(this.gameManager.getAliveUnits(0), this.unitTapWorldPoint.x, this.unitTapWorldPoint.z, bestDistanceSq);
          ...
          var bResult = this.pickClosestAliveUnitInList(this.gameManager.getAliveUnits(1), this.unitTapWorldPoint.x, this.unitTapWorldPoint.z, bestDistanceSq);
        ...
        pickClosestAliveUnitInList(units, x, z, maxDistanceSq) {
          ...
          for (var i = 0; i < units.length; i++) {
            ...
            var pos = unit.agent ? unit.agent.pos : null;
            var ux = 0;
            var uz = 0;
            ...
            var pickRadius = Math.max(this.unitTapPickRadius, unit.radius);
        ...
        screenPointToBattlePlane(screenX, screenY, out) {
          ...
          var origin = ray.o || ray.origin;
          var dir = ray.d || ray.direction;
          if (!origin || !dir) return false;
          if (Math.abs(dir.y) <= 0.00001) return false;
          var t = (this.unitTapPickPlaneY - origin.y) / dir.y;
          if (t < 0) return false;
        ...
        isTapWithinMoveThreshold(startX, startY, endX, endY) {
          var dx = endX - startX;
          var dy = endY - startY;
          var threshold = Math.max(0, this.unitTapMaxMovePixels);
