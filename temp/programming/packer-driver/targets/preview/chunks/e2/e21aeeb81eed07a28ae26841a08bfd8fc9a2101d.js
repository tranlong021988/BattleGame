    ...
    execute: function () {
      ...
      _export("RVOWorkerSimulator", RVOWorkerSimulator = class RVOWorkerSimulator {
        ...
        step(deltaTime, maxSubStepDeltaTime) {
          ...
          var totalDeltaTime = this.getSafeTotalDeltaTime(deltaTime);
          var maxStepDeltaTime = this.getSafeMaxSubStepDeltaTime(maxSubStepDeltaTime);
          var subSteps = Math.max(1, Math.min(32, Math.ceil(totalDeltaTime / maxStepDeltaTime)));
          var safeDeltaTime = totalDeltaTime / subSteps;
        ...
        sendObstaclesToWorker() {
          ...
          try {
            ...
            console.warn('[RVOWorkerSimulator] Failed to send obstacles; using main-thread fallback.', err);
            this.activateMainThreadFallback();
            return false;
          }
        ...
        createWorker() {
          ...
          try {
            ...
            worker = this.createNamedWorker(url, 'RVOWorkerSimulator');
          } catch (err) {
            console.warn('[RVOWorkerSimulator] Worker could not be created; using main-thread fallback.', err);
            this.activateMainThreadFallback();
            return;
          } finally {
            if (url) {
          ...
          var generation = ++this.workerGeneration;
          this.worker = worker;
          ...
          worker.onmessageerror = err => {
            ...
            console.warn('[RVOWorkerSimulator] Worker message failed; using main-thread fallback.', err);
            this.activateMainThreadFallback();
          };
        }
