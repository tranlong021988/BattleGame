      ...
      _export("SpectorDebugger", SpectorDebugger = (_dec = ccclass('SpectorDebugger'), _dec2 = property({
        ...
        captureNextFrame() {
          ...
          return _asyncToGenerator(function* () {
            if (!_this.enableSpector || !_this.isBrowser()) {
              return;
            }

            var spector = yield _this.ensureSpector();

            if (!spector || _this.destroyed) {
              return;
            }

            var canvas = _this.findRenderCanvas();

            var commandCount = Math.max(1, Math.floor(_this.captureCommandCount));
            ...
            if (typeof spector.captureCanvas === 'function') {
              ...
              _this.debugLog("Canvas command capture requested. commands=" + commandCount);
            ...
            if (typeof spector.captureNextFrame === 'function') {
              ...
              _this.debugLog('Next-frame capture requested.');
        ...
        initializeSpector() {
          ...
          return _asyncToGenerator(function* () {
            var spector = yield _this2.ensureSpector();

            if (!spector || _this2.destroyed) {
              return;
            }

            if (_this2.spyCanvasesOnStart && typeof spector.spyCanvases === 'function') {
              spector.spyCanvases();

              _this2.debugLog('Canvas spying enabled.');
            }

            if (_this2.showUIOnStart && typeof spector.displayUI === 'function') {
              spector.displayUI();

              _this2.debugLog('UI opened.');
            }

            if (_this2.captureOnStart) {
              window.setTimeout(() => _this2.captureNextFrame(), 0);
            }
          })();
        }
        ...
        ensureSpector() {
          ...
          return _asyncToGenerator(function* () {
            if (_this3.spector) {
              return _this3.spector;
            }

            if (_this3.loadingPromise) {
              return _this3.loadingPromise;
            }

            _this3.loadingPromise = _this3.loadSpector();
            return _this3.loadingPromise;
          })();
        }
        ...
        loadSpector() {
          ...
          return _asyncToGenerator(function* () {
            try {
              ...
              var spectorModule = yield _context.import('spectorjs');
              var namespace = (_spectorModule$defaul = spectorModule == null ? void 0 : spectorModule.default) != null ? _spectorModule$defaul : spectorModule;
              var SpectorCtor = (_ref = (_namespace$Spector = namespace == null ? void 0 : namespace.Spector) != null ? _namespace$Spector : spectorModule == null ? void 0 : spectorModule.Spector) != null ? _ref : (_SPECTOR = window.SPECTOR) == null ? void 0 : _SPECTOR.Spector;

              if (!SpectorCtor) {
                throw new Error('SPECTOR.Spector constructor was not found.');
              }

              _this4.spector = new SpectorCtor();

              _this4.patchCanvasCaptureDefault(_this4.spector);

              _this4.installCaptureExport(_this4.spector);

              _this4.debugLog('SpectorJS loaded.');

              return _this4.spector;
            } catch (error) {
              _this4.loadingPromise = null;
        ...
        patchCanvasCaptureDefault(spector) {
          ...
          var originalCaptureCanvas = spector.captureCanvas.bind(spector);

          spector.captureCanvas = function (canvas, commandCount, quickCapture, fullCapture) {
            ...
            var safeCommandCount = commandCount > 0 ? commandCount : Math.max(1, Math.floor(_this5.captureCommandCount));
        ...
        downloadCaptureJson(capture) {
          try {
            var json = JSON.stringify(capture);
...
//# sourceMappingURL=363837e4c6425e8b320d5631300b8ae21ca09c0c.js.map