    ...
    execute: function () {
      ...
      _export("BattleCardRuntime", BattleCardRuntime = class BattleCardRuntime {
        ...
        shouldActivate(card, team, currentCombatPoint, initialCombatPoint) {
          ...
          const safeTeam = this.clampTeam(team);
          const initial = Math.max(1, Number.isFinite(initialCombatPoint[safeTeam]) ? initialCombatPoint[safeTeam] : 1);
          const current = Math.max(0, Number.isFinite(currentCombatPoint[safeTeam]) ? currentCombatPoint[safeTeam] : 0);
